...
if($websitetitle == ""){
	...
	<html>
		...
		<body>
							...
							while($row = mysqli_fetch_assoc($result)){
								...
								<div class="filmblock">
									...
									<div>
										...
										<h2 style="margin-top: 20px;" class="producttitle"><?php echo shorten_text($row["title"], 25, ' ...', false) ?></h2><div class="realproducttitle" style="display: none"><?php echo $row["title"] ?></div><div class="productoptions" style="display: none"><?php echo $row["options"] ?></div><div class="homeproductprice" style="padding-bottom: 20px; font-weight: bold; color: <?php echo $maincolor ?>"><?php echo $oldprice . $currencysymbol . "<span class='thiscurrentpricedisplay'>" . number_format($saleprice) ?></span><span style="display: none;" class="thiscurrentprice"><?php echo $saleprice ?></span> <span style="font-size: 12px;">x</span> <input class="productquantity" type="number" value=1 min=1 style="vertical-align: middle; display: inline-block; width: 40px; padding: 2px; margin: 5px; border-radius: 0px;"></div>
			...
			<div id="dock">
				<div class="dockitem" onclick="openHome(); highlight(0);"><i class="fa fa-home"></i><span style="display: block; font-size: 14px;">Home</span></div>
				<div class="dockitem" onclick="showcartui(); highlight(1);"><i class="fa fa-shopping-cart"></i>
				<div style="position: absolute; top: 0; text-align: center; font-size: 20px; left: 5px; width: 100%; right: 0; padding: 5px; font-weight: bold;" class="cartcount"></div>
				<span style="display: block; font-size: 14px;">Cart</span>
				</div>
				<div class="dockitem" onclick="showabout(); highlight(2);"><i class="fa fa-info"></i><span style="display: block; font-size: 14px;">Info</span></div>
			</div>
			
			<script>
			
				function highlight(n){
					$(".dockitem").css({ "color" : "gray" })
					$(".dockitem").find(".fa").css({ "color" : "gray" })
					$(".dockitem").eq(n).css({ "color" : "white" })
					$(".dockitem").eq(n).find(".fa").css({ "color" : "<?php echo $maincolor ?>" })
				}
